...
Server_XPoWeR = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_XPoWeR.."Token_DevXPoWeR") then
...
redis:set(Server_XPoWeR.."Token_DevXPoWeR",token)
end 
else
...
------------------------------------------------------------------------------------------------------------
local DevXPoWeR_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevXPoWeR_Info_Sudo:write([[
do 
local XPoWeR_INFO = {
Id_DevXPoWeR = ]]..redis:get(Server_XPoWeR.."Id_DevXPoWeR")..[[,
UserName_XPoWeR = "]]..redis:get(Server_XPoWeR.."User_DevXPoWeR1")..[[",
Token_Bot = "]]..redis:get(Server_XPoWeR.."Token_DevXPoWeR")..[["
}
return XPoWeR_INFO
end

]])
DevXPoWeR_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_XPoWeR = io.open("XPoWeR", 'w')
Run_File_XPoWeR:write([[
#!/usr/bin/env bash
cd $HOME/XPoWeR
token="]]..redis:get(Server_XPoWeR.."Token_DevXPoWeR")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./XPoWeR.lua -p PROFILE --bot=$token
done
]])
Run_File_XPoWeR:close()
------------------------------------------------------------------------------------------------------------
...
screen -S XPoWeR ./XPoWeR
done
]])
Run_SM:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x XPoWeR')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_XPoWeR.."Token_DevXPoWeR");redis:del(Server_XPoWeR.."Id_DevXPoWeR");redis:del(Server_XPoWeR.."User_DevXPoWeR1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_XPoWeR
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevXPoWeR
Ids_Dev = {sudos.Id_DevXPoWeR,bot_id}
...
------------------------------------------------------------------------------------------------------------
function Dev_XPoWeR(msg)  
local Dev_XPoWeR = false  
...
function Dev_XPoWeR_User(user)  
local Dev_XPoWeR_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_XPoWeR_User = true  
end  
end  
return Dev_XPoWeR_User  
end 
function DeveloperBot(msg)  
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_XPoWeR(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_XPoWeR(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_XPoWeR_User(user_id) then
Status = true  
...
if Dev_XPoWeR_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/XPoWerEx/XPoWeR/master/XPoWeR.lua'
...
data.first_name_ = gmatch or 'XPoWeR'
end
...
------------------------------------------------------------------------------------------------------------
function FilesXPoWeR(msg)
File_Bot = dofile("XPoWeR.lua")
if File_Bot.XPoWeR and msg then
Text_File = File_Bot.XPoWeR(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesXPoWeRBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.XPoWeRFile and msg then
Text_FileBot = Text_FileBot.XPoWeRFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_XPoWeR_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevXPoWeR'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevXPoWeR'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevXPoWeR',text)
redis:del(bot_id..'GetTexting:DevXPoWeR'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_XPoWeR(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."XPoWeR:UsersBot")
local Groups = redis:smembers(bot_id..'XPoWeR:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'XPoWeR:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'XPoWeR:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'XPoWeR:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'XPoWeR:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/XPoWerEx/XPoWeR/master/XPoWeR.lua','XPoWeR.lua') 
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/XPoWerEx/XPoWeRFiles/master/XPoWeRFiles/"..File_Name)
if Res == 200 then
...
if not Dev_XPoWeR(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:DevXPoWeR'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DevXPoWeR')
...
if Dev_XPoWeR_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..XPoWeR..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_XPoWeR(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_XPoWeR(msg) or text == 'جلب نسخه الكروبات' and Dev_XPoWeR(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
File:write(t)
File:close()
sendDocument(msg.chat_id_, msg.id_,'./libs/'..bot_id..'.json', '📮┇ عدد مجموعات التي في البوت { '..#list..'}')
end
...
if text == ("مسح قائمه العام") and Dev_XPoWeR(msg) or text == ("مسح المحظورين عام") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
elseif text == ("قائمه العام") and Dev_XPoWeR(msg) or text == ("المحظورين عام") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
if Dev_XPoWeR_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
if Dev_XPoWeR_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_XPoWeR(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @b666p')   
return false
end
...
elseif text == "تفعيل المغادره" and Dev_XPoWeR(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_XPoWeR(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_XPoWeR(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_XPoWeR(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_XPoWeR(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_XPoWeR(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_XPoWeR(msg) then
redis:set(bot_id..'GetTexting:DevXPoWeR'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_XPoWeR(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_XPoWeR(msg) then 
...
elseif text == ("ردود المطور") and Dev_XPoWeR(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_XPoWeR(msg) then 
...
elseif text == "حذف رد للكل" and Dev_XPoWeR(msg) then 
...
if Dev_XPoWeR_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_XPoWeR(msg) or text == "تغيير اسم البوت" and Dev_XPoWeR(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_XPoWeR(msg) then 
...
𓂅 .XPoWeR TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/SourceXPower)

𓂅 . [Source Info ](t.me/SourceXPower)     

𓂅 . [XPoWeR iNDT](t.me/SourceXPower)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS XPoWeR](t.me/X_PoWerBoT)     
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_XPoWeR(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_XPoWeR(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_XPoWeR(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevXPoWeR = redis:get(bot_id..'Texting:DevXPoWeR')
if TextingDevXPoWeR then 
send(msg.chat_id_, msg.id_,TextingDevXPoWeR)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_XPoWeR(msg) then
redis:del(bot_id..'Texting:DevXPoWeR')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_XPoWeR(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_XPoWeR_File(msg,data)
FilesXPoWeRBot(msg,data)
